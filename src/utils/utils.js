...
axios.interceptors.response.use((res) => {
    if(res.data.error&&res.data.error==='由于您长时间未登录，请重新登录！'){
...
export function changeNumberMin(Array, tickSize) {
    let arr = []
    ...
    Array.forEach((item) => {
        arr.push({
            ...
            low: +(item[3]).toFixed(tickSize),
            open: +(item[1]).toFixed(tickSize),
            date: new Date(moment(item[0], 'YYYYMMDDHHmm').format('YYYY-MM-DD-HH-mm')),
            volume: item[5]
        })
