...
import { oneLevelCondition, twoLevelCondition, strategyList, submitTradeSet ,strategyDetail} from '../../../../../serivce'
...
export default class TradeSet extends Component {
    ...
    getOneLevelCondition() {
        oneLevelCondition({}).then(res => {
            this.setState({ oneLevelConditionList: res })
        })
    }
    //一级优先买入条件
    getTwoLevelCondition() {
        twoLevelCondition({}).then(res => {
            this.setState({ twoLevelConditionList: res })
        })
    }
    //获取策略列表
    getStrategyList() {
