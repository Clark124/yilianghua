...
import { editStep4, testBack } from '../../../../../serivce'
...
export default class RiskSet extends Component {
    ...
    componentWillMount() {
        ...
        if (data) {
            ...
            const { comboName, comboStyle, initFund, payPrice, description } = this.props.selectStockComboxInfo
            if (this.props.riskSetting && this.props.riskSetting.isEnable) {
                const { moveStopProfitRatio, stopLossRatio, stopProfitRatio } = this.props.riskSetting
                this.setState({ selectRiskIndex: 1, stopProfit: stopProfitRatio, stopLoss: stopLossRatio, moveStopProfit: moveStopProfitRatio })
            } else {
                this.setState({ selectRiskIndex: 0 })
            }
            this.setState({
                composeName: comboName,
                capital: initFund / 10000,
                price: payPrice,
                introduce: description,
                slectStyleList: JSON.parse(comboStyle)
    ...
    render() {
        const { styleList, slectStyleList, selectRiskIndex, stopProfit, stopLoss, moveStopProfit,
            composeName, capital, price, introduce, status,
            beginTime, endTime, cost } = this.state
        let costValue = ""
        if (cost === 0.0005) {
            costValue = '万分之五'
        } else if (cost === 0.0008) {
