...
import { deleteTrust, suspendDeploy, setTradeMethod, strategyNotice } from '../../../../serivce'
    ...
    deleteStrategy(item, index) {
        Modal.confirm({
            ...
            onOk: () => {
                ...
                deleteTrust({ token, id: item.id }).then(res => {
                    if (res.success) {
                        ...
                        {dataList.map((item, index) => {
                            return (
                                <tr className="table-item  trust" key={index}>
                                    <td className="item-name click" onClick={()=>this.props.history.push(`/strategy/detail/${item.id}`)}>{item.r_name.slice(0, -16)}</td>
